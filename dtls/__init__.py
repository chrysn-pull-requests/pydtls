...
def _prep_bins():
    ...
    try:
        exec(compile(open(path.join(prebuilt_path, "manifest.pycfg")).read(), path.join(prebuilt_path, "manifest.pycfg"), 'exec'), config)
    except IOError:
        return  # there are no prebuilts for this platform - nothing to do
    files = [path.join(prebuilt_path, x) for x in config["FILES"]]
